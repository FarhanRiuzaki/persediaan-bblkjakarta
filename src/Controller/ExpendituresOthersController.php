...
/**
 * ExpendituresOthers Controller
 *
 * @property \App\Model\Table\ExpendituresOthersTable $ExpendituresOthers
 *
 * @method \App\Model\Entity\ExpendituresOther[]|\Cake\Datasource\ResultSetInterface paginate($object = null, array $settings = [])
 */
class ExpendituresOthersController extends AppController
{
    private $titleModule = "Pengeluaran Barang Lainnya";
    private $primaryModel = "ExpendituresOthers";
    ...
    public function view($id = null)
    {
        ...
        if (empty($request_print)) {
            $expendituresOther = $this->ExpendituresOthers->get($id, [
                'contain' => ['ExpendituresOthersDetails', 'ExpendituresOthersDetails.Products', 'CreatedUsers', 'ModifiedUsers']
            ]);
            $this->set('expendituresOther', $expendituresOther);
        } else {
            $expendituresOther = $this->ExpendituresOthers->get($id, [
                'contain' => ['ExpendituresOthersDetails', 'ExpendituresOthersDetails.Products', 'CreatedUsers', 'ModifiedUsers']
            ]);
        ...
        $titleModule = 'Pengeluaran Barang Lainnya';
    ...
    public function add()
    { 
        $expendituresOther = $this->ExpendituresOthers->newEntity();
        if ($this->request->is('post')) {
            $data = $this->request->getData();
            $data['date'] = $this->Utilities->generalitationDateFormat($data['date']);

            $expendituresOther = $this->ExpendituresOthers->patchEntity($expendituresOther, $data, [
                'associated' => [
                    'ExpendituresOthersDetails'
                ]
            ]);

            // dd($expendituresOther);
            if ($this->ExpendituresOthers->save($expendituresOther)) {
                $code = 200;
                $message = __('Pengeluaran barang lainnya berhasil disimpan.');
            } else {
                $code = 50;
                $message = __('Pengeluaran barang lainnya gagal disimpan. Silahkan ulangi kembali.');
            }

            $this->set(compact('code', 'message', 'receive', 'data'));
            $this->set('_serialize', ['code', 'message', 'expendituresOther', 'data']);
        }

        $titleModule = 'Pengeluaran Barang Lainnya';
        $titlesubModule = 'Tambah Pengeluaran Barang Lainnya';
        $breadCrumbs = [
            Router::url(['action' => 'index']) => $titleModule,
            Router::url(['action' => 'add']) => $titlesubModule
        ];

        $this->set(compact('titleModule', 'breadCrumbs', 'titlesubModule', 'expendituresOther'));
    }
    ...
    {
        $expendituresOther = $this->ExpendituresOthers->get($id, [
            'contain' => ['ExpendituresOthersDetails', 'ExpendituresOthersDetails.Products']
        ]);
            ...
            if ($this->ExpendituresOthers->save($expendituresOther)) {
                if (!empty($data['delete_detail'])) {
                    ...
                    foreach ($explodeId as $id) {
                        $ExpendituresOthersDetails = $this->ExpendituresOthers->ExpendituresOthersDetails->get($id);
                        $this->ExpendituresOthers->ExpendituresOthersDetails->delete($ExpendituresOthersDetails);
                    }
                }
                $code = 200;
                $message = __('Pengeluaran barang lainnya berhasil disimpan.');
        ...
        $expendituresOther->date = $expendituresOther->date->format('d-m-Y');

        $titleModule = 'Pengeluaran Barang Lainnya';
        $titlesubModule = 'Edit Pengeluaran Barang Lainnya';
        $breadCrumbs = [
            Router::url(['action' => 'index']) => 'List ' . $titleModule,
            Router::url(['action' => 'update']) => $titlesubModule
        ];

        $this->set(compact('titleModule', 'breadCrumbs', 'titlesubModule', 'expendituresOther', 'institutes'));
    }
