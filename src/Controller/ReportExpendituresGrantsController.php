 ...
 */
class ReportExpendituresGrantsController extends AppController
{
    public function index()
    {
        $request_print = $this->request->query('print');
        $titleModule = 'Laporan Pengeluaran Barang Hibah';
        if (empty($request_print)) {
            ...
            $start = $startDay.' '.$arrayMonth[$startMonth*1].' '.$startYear;

            $endYear = date('Y', strtotime($end));
            $endMonth = date('m', strtotime($end));
            $endDay = date('d', strtotime($end));
            $end = $endDay.' '.$arrayMonth[$endMonth*1].' '.$endYear;

            $periode = $start . ' s.d ' . $end;
            $titleModule = 'Laporan Pengeluaran Barang Hibah ' . $periode;

            $this->loadModel('ExpendituresGrantsDetails');
            $results = $this->ExpendituresGrantsDetails->find('all', [
                'contain' => [
                    'Products',
                    'ExpendituresGrants',
                ]
            ])->select([
                'code1' => 'ExpendituresGrants.code',
                'given_to' => 'ExpendituresGrants.given_to',
                'code' => 'Products.code',
                'unit' => 'Products.unit',
                'name' => 'Products.name',
                'date' => 'ExpendituresGrants.date',
                'qty' => 'ExpendituresGrantsDetails.qty',
                'price' => 'ExpendituresGrantsDetails.price',
                'subtotal' => '(ExpendituresGrantsDetails.price * ExpendituresGrantsDetails.qty)',
            ])->where([
                'DATE(ExpendituresGrants.date) BETWEEN "' . $start . '" AND "' . $end . '"'
            ]);
            ...
            if ($request_print == 'pdf') {
                ...
                $sheet->setCellValue('A1', 'LAPORAN PENGELUARAN BARANG HIBAH');
                ...
                $sheet->setCellValue('B' . $start, 'Kode Pengeluaran Barang Hibah');
                ...
                foreach ($results as $grant) {
                    ...
                    $sheet->setCellValue('C' . $col, $grant->given_to);
                ...
                header('Content-Disposition: attachment;filename="Laporan Pengeluaran Barang Hibah' . rand(2, 32012) . '.xlsx"');
