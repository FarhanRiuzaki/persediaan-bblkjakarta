 ...
 */
class ReportExpendituresOthersController extends AppController
{
    public function index()
    {
        $request_print = $this->request->query('print');
        $titleModule = 'Laporan Pengeluaran Barang Lainnya';
        if (empty($request_print)) {
            ...
            $titleModule = 'Laporan Pengeluaran Barang Lainnya ' . $periode;

            $this->loadModel('ExpendituresOthersDetails');
            $results = $this->ExpendituresOthersDetails->find('all', [
                'contain' => [
                    'Products',
                    'ExpendituresOthers',
                ]
            ])->select([
                'code1' => 'ExpendituresOthers.code',
                'given_to' => 'ExpendituresOthers.given_to',
                'code' => 'Products.code',
                'unit' => 'Products.unit',
                'name' => 'Products.name',
                'date' => 'ExpendituresOthers.date',
                'qty' => 'ExpendituresOthersDetails.qty',
                'price' => 'ExpendituresOthersDetails.price',
                'subtotal' => '(ExpendituresOthersDetails.price * ExpendituresOthersDetails.qty)',
            ])->where([
                'DATE(ExpendituresOthers.date) BETWEEN "' . $start . '" AND "' . $end . '"'
            ]);
            ...
            if ($request_print == 'pdf') {
                ...
                $sheet->setCellValue('A1', 'LAPORAN PENGELUARAN BARANG LAINNYA');
                ...
                $sheet->setCellValue('B' . $start, 'Kode Pengeluaran Barang Lainnya');
                $sheet->setCellValue('C' . $start, 'Diberikan Kepada');
                ...
                foreach ($results as $other) {
                    $total_qty += $other->qty;
                    $total_price += $other->price;
                    $total_subtotal += $other->subtotal;

                    $sheet->setCellValue('A' . $col, $no);
                    $sheet->setCellValue('B' . $col, $other->code1);
                    $sheet->setCellValue('C' . $col, $other->given_to);
                    $sheet->setCellValue('D' . $col, $other->code . '-' . $other->name);
                    $sheet->setCellValue('E' . $col, number_format($other->qty) . ' ' . $other->unit);
                    $sheet->setCellValue('F' . $col, number_format($other->price));
                    $sheet->setCellValue('G' . $col, number_format($other->subtotal));
                    $no++;
                    $col++;
                }
                ...
                header('Content-Disposition: attachment;filename="Laporan Pengeluaran Barang Lainnya' . rand(2, 32012) . '.xlsx"');
