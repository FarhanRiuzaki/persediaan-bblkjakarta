 ...
 */
class ReportReceiptOthersController extends AppController
{
    public function index()
    {
        $request_print = $this->request->query('print');
        $titleModule = 'Laporan Penerimaan Barang Lainnya';
        if (empty($request_print)) {
            ...
            $titleModule = 'Laporan Penerimaan Barang Lainnya ' . $periode;

            $this->loadModel('ReceiptOthersDetails');
            $results = $this->ReceiptOthersDetails->find('all', [
                'contain' => [
                    'Products',
                    'ReceiptOthers',
                ]
            ])->select([
                'code1' => 'ReceiptOthers.code',
                'source' => 'ReceiptOthers.source',
                'code' => 'Products.code',
                'unit' => 'Products.unit',
                'name' => 'Products.name',
                'date' => 'ReceiptOthers.date',
                'qty' => 'ReceiptOthersDetails.qty',
                'price' => 'ReceiptOthersDetails.price',
                'subtotal' => '(ReceiptOthersDetails.price * ReceiptOthersDetails.qty)',
            ])->where([
                'DATE(ReceiptOthers.date) BETWEEN "' . $start . '" AND "' . $end . '"'
            ]);
            ...
            if ($request_print == 'pdf') {
                ...
                $sheet->setCellValue('A1', 'LAPORAN PENERIMAAN BARANG LAINNYA');
                ...
                $sheet->setCellValue('B' . $start, 'Kode Penerimaan Barang Lainnya');
                ...
                header('Content-Disposition: attachment;filename="Laporan Penerimaan Barang Lainnya' . rand(2, 32012) . '.xlsx"');
