 ...
 */
class ReportReceiptTransfersController extends AppController
{
    public function index()
    {
        $request_print = $this->request->query('print');
        $titleModule = 'Laporan Penerimaan Barang Transfer Antar Lembaga';
        if (empty($request_print)) {
            ...
            $titleModule = 'Laporan Penerimaan Barang Transfer Antar Lembaga ' . $periode;

            $this->loadModel('ReceiptTransfersDetails');
            $results = $this->ReceiptTransfersDetails->find('all', [
                'contain' => [
                    'Products',
                    'ReceiptTransfers',
                    'ReceiptTransfers.Institutions'
                ]
            ])->select([
                'code1' => 'ReceiptTransfers.code',
                'lembaga' => 'Institutions.name',
                'code_lembaga' => 'Institutions.code',
                'code' => 'Products.code',
                'unit' => 'Products.unit',
                'name' => 'Products.name',
                'date' => 'ReceiptTransfers.date',
                'qty' => 'ReceiptTransfersDetails.qty',
                'price' => 'ReceiptTransfersDetails.price',
                'subtotal' => '(ReceiptTransfersDetails.price * ReceiptTransfersDetails.qty)',
            ])->where([
                'DATE(ReceiptTransfers.date) BETWEEN "' . $start . '" AND "' . $end . '"'
            ]);
            ...
            if ($request_print == 'pdf') {
                ...
                $sheet->setCellValue('A1', 'LAPORAN PENERIMAAN BARANG TRANSFER ANTAR LEMBAGA');
                ...
                $sheet->setCellValue('B' . $start, 'Kode Penerimaan Barang Transfer Antar Lembaga');
                ...
                header('Content-Disposition: attachment;filename="Penerimaan Barang Transfer Antar Lembaga' . rand(2, 32012) . '.xlsx"');
