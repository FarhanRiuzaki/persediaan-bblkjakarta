                                ...
                                <div class="col-md-12">
                                    <table class="table table-condensed table-bordered table-input">
                                        <thead>
                                            <tr>
                                                <th width="30px">No.</th>
                                                <th width="300px">Nama Unit Kerja</th>
                                        ...
                                        <tbody>
                                        <?php foreach ($record->purchase_requests_details as $key => $r):?>
                                        <tr>
                                            <td class="number"><?=$key + 1;?></td>
                                            <td><?=$record->institute->name;?></td>
                                            <td><?=$r->product->name;?></td>
                                            <!-- <td><?=$this->Number->format($r['saldo'], ['precision' => 2]);?></td> -->
                                            <td class="text-center"><?=$r->product->unit;?></td>
                                            <td class="text-right"><?=$r->qty;?></td>
                                            <td>
                                                <input type="text" class="form-control onlyNumberWithoutSeparator" data-name="qty" value="<?=$r->qty;?>" name="purchase_requests_details[<?=$key;?>][qty]">
                                                <input type="hidden" class="form-control onlyNumberWithoutSeparator" data-name="qty" value="<?=$r->qty;?>" name="purchase_requests_details[<?=$key;?>][qty_request]">
                                                
                                                <input type="hidden" data-name="id" value="<?=$r['id'];?>" name="purchase_requests_details[<?=$key;?>][id]">
                                                <input type="hidden" data-name="status" value="0" name="purchase_requests_details[<?=$key;?>][status]">
                                                <input type="hidden" data-name="product_id" value="<?=$r['product_id'];?>" name="purchase_requests_details[<?=$key;?>][product_id]">
                                                <input type="hidden" data-name="purchase_request_id" value="<?=$r['purchase_request_id'];?>" name="purchase_requests_details[<?=$key;?>][purchase_request_id]">
                                            </td>
                                            <td><a href="#" class="btn btn-danger btn-delete-detail" data-id="<?=$r['purchase_request_id'];?>"><i class="fa fa-times"></i></a></td>
                                        </tr>
                                        <?php endforeach;?>
                                        </tbody>
                                    </table>
