<style>
    ...
    <div class="m-portlet__body">
        ...
        <table class='body' width="100%;" style="font-size: 15px !important;">
            ...
            <?php if (!empty($internalOrder->purchase_requests_details )): ?>
                <?php foreach ($internalOrder->purchase_requests_details  as $val): ?>
                <tr>
                    <td><?= h($no += 1) ?></td>
                    <td><?= h($val->product->name) ?></td>
                    <td class="text-center"><?= h($val->qty) ?> <?= h($val->product->unit) ?></td>
                    <td></td>
                </tr>
                <?php endforeach; ?>
            <?php endif; ?>
        ...
        <table class='footer text-center' width="100%;" style="font-size: 15px !important;">
            <tr>
                <td rowspan="3" width='33%'>Tanggal: <?= $internalOrder->date->format('d-m-Y')?> <br>
                        Yang mengajukan:
