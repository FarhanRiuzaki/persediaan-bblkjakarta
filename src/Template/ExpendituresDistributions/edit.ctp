                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <table class="table table-condensed table-bordered table-input">
                                        ...
                                        <tbody>
                                            <?php foreach($expendituresDistribution->expenditures_distributions_details as $key => $r):?>
                                            <tr>
                                                <td class="number"><?=$key+1;?></td>
                                                <td>[<?=$r->product->code;?>] <?=$r->product->name;?></td>
                                                <td><?=$this->Number->format($r->saldo_product,['precision'=>2]);?></td>
                                                <td class="text-right"><?=$r->internal_orders_detail->qty;?></td>
                                                <td class="text-right"><?=$r->saldo_dist;?></td>
                                                <td>
                                                    <input type="text" class="form-control onlyNumberWithoutSeparator" data-name="qty" value="<?=$r->qty;?>" name="expenditures_distributions_details[<?=$key;?>][qty]">
                                                </td>
                                                <td>
                                                    <input type="text" class="form-control onlyNumber" data-name="price" value="<?=$r->price;?>" name="expenditures_distributions_details[<?=$key;?>][price]"><input type="hidden" data-name="product_id" value="<?=$r->product->id;?>" name="expenditures_distributions_details[<?=$key;?>][product_id]"><input type="hidden" data-name="internal_order_id" value="<?=$r->internal_order_id;?>" name="expenditures_distributions_details[<?=$key;?>][internal_order_id]"><input type="hidden" data-name="internal_orders_detail_id" value="<?=$r->internal_order_detail_id;?>" name="expenditures_distributions_details[<?=$key;?>][internal_orders_detail_id]"><input type="hidden" data-name="id" value="<?=$r->id;?>" name="expenditures_distributions_details[<?=$key;?>][id]">
                                                </td>
                                            </tr>
                                            <?php endforeach;?>
                                        </tbody>
                    ...
                    success: function(result){
                        ...
                        $.each(data, function(key, val) {
                            ...
                            var html =   "<tr>"
                                        ...
                                        +"</td>"
                                        +"<td>"
                                        +"<input type='text' class='form-control onlyNumberWithoutSeparator' data-name='qty' value='"+val.qty+"' name='expenditures_distributions_details["+countDetail+"][qty]'>"
                                        +"</td>"
                                        +"<td>"
            ...
            submitHandler: function (form) {
                ...
                if(validateDetail){
                    $(form).ajaxSubmit({
                        ...
                        success : function(result){
                            if(result.code == 200){
                                swal.fire('Berhasil diubah','Data distribusi barang berhasil diubah','success');
