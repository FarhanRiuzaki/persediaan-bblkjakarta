...
<div class="row">
    <div class="col-md-12">
        <div class="kt-portlet">
			...
			<!--begin::Form-->
            <?= $this->Form->create($expendituresOther,['class'=>'kt-form form-primary','type'=>'file']) ?>
            <div class="kt-portlet__body">
					<div class="kt-section kt-section--first">
                        <h3 class="kt-section__title">Pengeluaran Barang lainnya:</h3>
                        <div class="kt-section__body">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <?=$this->Form->control('given_to', ['type' => 'text', 'label' => 'Diberikan Kepada', 'class' => 'form-control']);?>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="col-md-12">
                                    <table class="table table-condensed table-bordered table-input">
                                        ...
                                        <tbody>
                                            ...
                                            <?php foreach ($expendituresOther->expenditures_others_details as $key => $expenditures_others_detail):?>
                                                <tr>
                                                    <td class="number"><?=$key + 1;?></td>
                                                    <td>[<?=$expenditures_others_detail->product->code;?>] <?=$expenditures_others_detail->product->name;?></td>
                                                    <td colspan="2" class="onlyNumberWithoutSeparator"><?=$expenditures_others_detail->qty;?></td>
                                                    <td class="onlyNumberPrice"><?=$expenditures_others_detail->price;?></td>
                                                    <td class="text-center">
                                                        <input type="hidden" data-name="id" value="<?=$expenditures_others_detail->id;?>" name="expenditures_others_details[<?=$key;?>][id]">
                                                        <input type="hidden" data-name="product_id" value="<?=$expenditures_others_detail->product_id;?>" name="expenditures_others_details[<?=$key;?>][product_id]">
                                                        <input type="hidden" data-name="qty" value="<?=$expenditures_others_detail->qty;?>" name="expenditures_others_details[<?=$key;?>][qty]">
                                                        <input type="hidden" data-name="price" value="<?=$expenditures_others_detail->price;?>" name="expenditures_others_details[<?=$key;?>][price]">
                                                        <a href="#" class="btn btn-danger btn-delete-detail" data-id="<?=$expenditures_others_detail->id;?>"><i class="fa fa-times"></i></a>
                                                    </td>
                                                </tr>
                                            <?php endforeach;?>
                                        </tbody>
...
<?php $this->start('script');?>
    <script>
        var arrayProduct = [];
        var oldProduct = [];
        var endDate = '<?=date('Y-m-d', strtotime($expendituresOther->date))?>';
        ...
        $(".btn-add-detail").on("click",function(e){
            ...
            if(valid){
                ...
                var html =   "<tr>"
                            ...
                            +"</td>"
                            +"<td>"
                            +"<input type='hidden' data-name='product_id' value='"+productId+"' name='expenditures_others_details["+countDetail+"][product_id]'>"
                            +"<input type='hidden' data-name='price' value='"+price+"' name='expenditures_others_details["+countDetail+"][price]'>"
                            +"<input type='hidden' data-name='qty' value='"+qty+"' name='expenditures_others_details["+countDetail+"][qty]'>"
        ...
        $("body").on("click",".btn-delete-detail",function(e){
            ...
            $.each($(".table-input tbody tr:not(.tr-input)"),function(e,item){
                ...
                $.each($(this).find("input"),function(s,f){
                    ...
                    $(this).attr("name","expenditures_others_details["+e+"]["+dataName+"]");
                })
                ...
                if(validateDetail){
                    $(form).ajaxSubmit({
                        ...
                        success : function(result){
                            if(result.code == 200){
                                swal.fire('Berhasil diubah','Data pengeluaran barang lainnya berhasil diubah','success');
