...
<div class="row">
    <div class="col-md-12">
        <div class="kt-portlet">
			...
			<!--begin::Form-->
            <?= $this->Form->create($expendituresReclarification,['class'=>'kt-form form-primary','type'=>'file']) ?>
                                ...
                                <div class="col-md-12">
                                    <table class="table table-condensed table-bordered table-input">
                                        ...
                                        <tbody class="container-detail">
                                            ...
                                            <?php foreach ($expendituresReclarification->expenditures_reclarifications_details as $key => $expenditures_reclarifications_detail):?>
                                                <tr>
                                                    <td class="number"><?=$key + 1;?></td>
                                                    <td>[<?=$expenditures_reclarifications_detail->product->code;?>] <?=$expenditures_reclarifications_detail->product->name;?></td>
                                                    <td colspan="2" class="onlyNumberWithoutComa"><?=$expenditures_reclarifications_detail->qty;?></td>
                                                    <td class="onlyNumberPrice"><?=$expenditures_reclarifications_detail->price;?></td>
                                                    <td class="text-center">
                                                        <input type="hidden" data-name="id" value="<?=$expenditures_reclarifications_detail->id;?>" name="expenditures_reclarifications_details[<?=$key;?>][id]">
                                                        <input type="hidden" data-name="product_id" value="<?=$expenditures_reclarifications_detail->product_id;?>" name="expenditures_reclarifications_details[<?=$key;?>][product_id]">
                                                        <input type="hidden" data-name="price" value="<?=$expenditures_reclarifications_detail->price;?>" name="expenditures_reclarifications_details[<?=$key;?>][price]">
                                                        <input type="hidden" data-name="qty" value="<?=$expenditures_reclarifications_detail->qty;?>" name="expenditures_reclarifications_details[<?=$key;?>][qty]">
                                                        <a href="#" class="btn btn-danger btn-delete-detail" data-id="<?=$expenditures_reclarifications_detail->id;?>"><i class="fa fa-times"></i></a>
                                                    </td>
                                                </tr>
                                            <?php endforeach;?>
                                        </tbody>
...
<?php $this->start('script');?>
    <script>
        var arrayProduct = [];
        var oldProduct = [];
        var endDate = '<?=date('Y-m-d', strtotime($expendituresReclarification->date))?>';
        ...
        $(".btn-add-detail").on("click",function(e){
            ...
            if(valid){
                ...
                var html =   "<tr>"
                            ...
                            +"</td>"
                            +"<td>"
                            +"<input type='hidden' data-name='product_id' value='"+productId+"' name='expenditures_reclarifications_details["+countDetail+"][product_id]'>"
                            +"<input type='hidden' data-name='price' value='"+price+"' name='expenditures_reclarifications_details["+countDetail+"][price]'>"
                            +"<input type='hidden' data-name='qty' value='"+qty+"' name='expenditures_reclarifications_details["+countDetail+"][qty]'>"
        ...
        $("body").on("click",".btn-delete-detail",function(e){
            ...
            $.each($(".table-input tbody tr:not(.tr-input)"),function(e,item){
                ...
                $.each($(this).find("input"),function(s,f){
                    ...
                    $(this).attr("name","expenditures_reclarifications_details["+e+"]["+dataName+"]");
                })
                ...
                if(validateDetail){
                    $(form).ajaxSubmit({
                        ...
                        success : function(result){
                            if(result.code == 200){
                                swal.fire('Berhasil diubah','Data pengeluaran reklarifikasi berhasil diubah','success');
