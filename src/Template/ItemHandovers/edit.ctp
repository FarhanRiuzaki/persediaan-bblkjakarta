        ...
        <div class="kt-portlet">
            ...
            <div class="kt-portlet__body">
					<div class="kt-section kt-section--first">
                        <h3 class="kt-section__title">Barang Keluar:</h3>
                                    ...
                                    <table class="table table-condensed table-bordered table-input">
                                        ...
                                        <tbody>
                                            <tr class="tr-input">
                                                ...
                                                <td>
                                                    <input type="text" id="price" class="price form-control onlyNumber" readonly>
                                                </td>
                                                <td class="text-center">
                                                    <a href="#" class="btn btn-info btn-add-detail"><i class="fa fa-check"></i></a>
                                                </td>
                                            </tr>
                                            <?php foreach ($record->item_handovers_details as $key => $val):?>
                                                <tr>
                                                    <td class="number"><?=$key + 1;?></td>
                                                    <td>[<?=$val->product->code;?>] <?=$val->product->name;?></td>
                                                    <td colspan="3" class="text-center"><?=$val->qty . ' ' . $val->product->unit?></td>
                                                    <td class="onlyNumberPrice"><?=number_format($val->price);?></td>
                                                    <td class="text-center">
                                                        <input type="hidden" data-name="id" value="<?=$val->id;?>" name="item_handovers_details[<?=$key;?>][id]">
                                                        <input type="hidden" data-name="product_id" value="<?=$val->product_id;?>" name="item_handovers_details[<?=$key;?>][product_id]">
                                                        <input type="hidden" data-name="qty" value="<?=$val->qty;?>" name="item_handovers_details[<?=$key;?>][qty]">
                                                        <input type="hidden" data-name="price" value="<?=$val->price;?>" name="item_handovers_details[<?=$key;?>][price]">
                                                        <a href="#" class="btn btn-danger btn-delete-detail" data-id="<?=$val->id;?>"><i class="fa fa-times"></i></a>
                                                    </td>
                                                </tr>
                                            <?php endforeach;?>
                                        </tbody>
...
<?php $this->start('script');?>
    <script>
        var arrayProduct = [];
        var oldProduct = [];
        var endDate = '<?=date('Y-m-d', strtotime($record->date))?>';
        ...
        $(".btn-add-detail").on("click",function(e){
            ...
            if(valid){
                ...
                var html =   "<tr>"
                            ...
                            +"</td>"
                            +"<td>"
                            +"<input type='hidden' data-name='product_id' value='"+productId+"' name='item_handovers_details["+countDetail+"][product_id]'>"
                            +"<input type='hidden' data-name='price' value='"+price+"' name='item_handovers_details["+countDetail+"][price]'>"
                            +"<input type='hidden' data-name='qty' value='"+qty+"' name='item_handovers_details["+countDetail+"][qty]'>"
        ...
        $("body").on("click",".btn-delete-detail",function(e){
            ...
            $.each($(".table-input tbody tr:not(.tr-input)"),function(e,item){
                ...
                $.each($(this).find("input"),function(s,f){
                    ...
                    $(this).attr("name","item_handovers_details["+e+"]["+dataName+"]");
                })
