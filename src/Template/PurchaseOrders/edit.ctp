					...
					<div class="kt-section kt-section--first">
                        ...
                        <div class="kt-section__body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <?=$this->Form->control('date_freeze', ['datepicker' => 'true', 'type' => 'text', 'label' => 'Tanggal Pembukuan', 'value' => $purchaseOrder['date_freeze'], 'class' => 'form-control']);?>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <table class="table table-condensed table-bordered table-input">
                                        ...
                                        <tbody>
                                            <?php if(!empty($purchaseOrder->purchase_orders_details)):?>
                                                <?php foreach($purchaseOrder->purchase_orders_details as $key => $r):?>
                                                <tr>
                                                    <td><?=$key+1;?></td>
                                                    <td><?=$r->purchase_requests_detail->purchase_request->institute->name;?></td>
                                                    <td><?=$r->purchase_requests_detail->purchase_request->code;?></td>
                                                    <td>[<?=$r->product->code;?>] <?=$r->product->name;?></td>
                                                    <td class="text-right"><?=$this->Number->format($r->purchase_requests_detail->qty,['precision'=>2]);?></td>
                                                    <td class="text-right"><?=$this->Number->format($r->saldo_po,['precision'=>2]);?></td>
                                                    <td class="text-right">
                                                        <input id="qty_<?=$key?>" type="text" class="onlyNumber form-control m-input" value="<?=$r->qty;?>" name="purchase_orders_details[<?=$key;?>][qty]">
                                                    </td>
                                                    <td class="text-right">
                                                        <input id="price_<?=$key?>" type="text" class="onlyNumber form-control m-input" value="<?=$r->price;?>" name="purchase_orders_details[<?=$key;?>][price]">
                                                        <input type="hidden" class="form-control m-input" value="<?=$r->purchase_requests_detail->purchase_request_id;?>" name="purchase_orders_details[<?=$key;?>][purchase_request_id]">
                                                        <input type="hidden" class="form-control m-input" value="<?=$r->purchase_requests_detail->id;?>" name="purchase_orders_details[<?=$key;?>][purchase_requests_detail_id]">
                                                        <input type="hidden" class="form-control m-input" value="<?=$r->product->id;?>" name="purchase_orders_details[<?=$key;?>][product_id]">
                                                        <input type="hidden" class="form-control m-input" value="<?=$r->id;?>" name="purchase_orders_details[<?=$key;?>][id]">
                                                    </td>
                                                    <td id="total_<?=$key?>"><?=$this->Number->format(($r->price * $r->qty), ['precision' => 2]);?></td>
                                                </tr>
                                                <?php endforeach;?>
                                            <?php else:?>
                                                <tr>
                                                    <td colspan="9">Belum tersedia pemesanan</td>
                                                </tr>
                                            <?php endif;?>
                                        </tbody>
                                    </table>
