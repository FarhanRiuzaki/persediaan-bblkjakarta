...
<div class="row">
    <div class="col-md-12">
        <div class="kt-portlet">
			...
			<!--begin::Form-->
            <?= $this->Form->create($receiptGrant,['class'=>'kt-form form-primary','type'=>'file']) ?>
            <div class="kt-portlet__body">
					<div class="kt-section kt-section--first">
                        <h3 class="kt-section__title">Penerimaan Barang Hibah:</h3>
                        <div class="kt-section__body">
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    echo $this->Form->control('date_bast', ['datepicker' => 'true', 'type' => 'text', 'label' => 'Tanggal BAST', 'autocomplete' => 'off', 'class' => 'form-control']);

                                    ?>
                                </div>
                                <div class="col-md-4">
                                    <?=$this->Form->control('code_bast', ['label' => 'No. BAST', 'class' => 'form-control', 'autocomplete' => 'off']);?>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    <?=$this->Form->control('source', ['type' => 'text', 'label' => 'Sumber Hibah', 'class' => 'form-control']);?>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="col-md-12">
                                    <table class="table table-condensed table-bordered table-input">
                                        ...
                                        <tbody>
                                            ...
                                            <?php foreach ($receiptGrant->receipt_grants_details as $key => $receiptGrant):?>
                                                <tr>
                                                    <td class="number"><?=$key + 1;?></td>
                                                    <td>[<?=$receiptGrant->product->code;?>] <?=$receiptGrant->product->name;?></td>
                                                    Pembelian
                                                    <td class="onlyNumberWithoutComa"><?=$receiptGrant->qty;?></td>
                                                    <td class="onlyNumberPrice"><?=$receiptGrant->price;?></td>

                                                    <td class="text-center">
                                                        <input type="hidden" data-name="id" value="<?=$receiptGrant->id;?>" name="receipt_grants_details[<?=$key;?>][id]">
                                                        <input type="hidden" data-name="product_id" value="<?=$receiptGrant->product_id;?>" name="receipt_grants_details[<?=$key;?>][product_id]">
                                                        <input type="hidden" data-name="qty" value="<?=$receiptGrant->qty;?>" name="receipt_grants_details[<?=$key;?>][qty]">

                                                        <input type="hidden" data-name="price" value="<?=$receiptGrant->price;?>" name="receipt_grants_details[<?=$key;?>][price]">

                                                        <a href="#" class="btn btn-danger btn-delete-detail" data-id="<?=$receiptGrant->id;?>"><i class="fa fa-times"></i></a>
...
<?php $this->start('script');?>
    <script>
        ...
        $(".btn-add-detail").on("click",function(e){
            ...
            if(valid){
                ...
                var html =   "<tr>"
                            ...
                            +"</td>"
                            +"<td>"
                            +"<input type='hidden' data-name='product_id' value='"+productId+"' name='receipt_grants_details["+countDetail+"][product_id]'>"
                            +"<input type='hidden' data-name='qty' value='"+qty+"' name='receipt_grants_details["+countDetail+"][qty]'>"
                             +"<input type='hidden' data-name='price' value='"+price+"' name='receipt_grants_details["+countDetail+"][price]'>"
        ...
        $("body").on("click",".btn-delete-detail",function(e){
            ...
            $.each($(".table-input tbody tr:not(.tr-input)"),function(e,item){
                ...
                $.each($(this).find("input"),function(s,f){
                    ...
                    $(this).attr("name","receipt_grants_details["+e+"]["+dataName+"]");
                })
            })
        ...
        var validatorPrimary = $(".form-primary").validate({
            ...
            messages: {
                code: {
                    required: "Harap masukan kode penerimaan barang hibah "
                },
                date: {
                    required: "Harap masukan tanggal"
                },
                source: {
                    required: "Harap masukkan sumber hibah"
                },
                date_bast : {
                    required: "Harap masukan tanggal BAST "
                },
                code_bast : {
                    required: "Harap masukan No. BAST "
                }
            },
                ...
                if(validateDetail){
                    $(form).ajaxSubmit({
                        ...
                        success : function(result){
                            if(result.code == 200){
                                swal.fire('Berhasil diubah','Data penerimaan barang hibah berhasil diubah','success');
