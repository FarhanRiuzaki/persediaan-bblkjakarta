...
<div class="row">
    <div class="col-md-12">
        <div class="kt-portlet">
			...
			<!--begin::Form-->
            <?= $this->Form->create($receiptOther,['class'=>'kt-form form-primary','type'=>'file']) ?>
            <div class="kt-portlet__body">
					<div class="kt-section kt-section--first">
                        <h3 class="kt-section__title">Penerimaan Barang Lainnya:</h3>
                        ...
                        <div class="kt-section__body">
                            <div class="row">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($receiptOther->receipt_others_details as $key => $receipt_others_detail):?>
                                        <tr>
                                            <td class="number"><?=$key + 1;?></td>
                                            <td>[<?=$receipt_others_detail->product->code;?>] <?=$receipt_others_detail->product->name;?></td>
                                            Pembelian
                                            <td class="onlyNumberWithoutComa"><?=$receipt_others_detail->qty;?></td>
                                            <td class="onlyNumberPrice"><?=$receipt_others_detail->price;?></td>

                                            <td class="text-center">
                                                <input type="hidden" data-name="id" value="<?=$receipt_others_detail->id;?>" name="receipt_others_details[<?=$key;?>][id]">
                                                <input type="hidden" data-name="product_id" value="<?=$receipt_others_detail->product_id;?>" name="receipt_others_details[<?=$key;?>][product_id]">
                                                <input type="hidden" data-name="qty" value="<?=$receipt_others_detail->qty;?>" name="receipt_others_details[<?=$key;?>][qty]">

                                                <input type="hidden" data-name="price" value="<?=$receipt_others_detail->price;?>" name="receipt_others_details[<?=$key;?>][price]">

                                                <a href="#" class="btn btn-danger btn-delete-detail" data-id="<?=$receipt_others_detail->id;?>"><i class="fa fa-times"></i></a>
        ...
        $(".btn-add-detail").on("click",function(e){
            ...
            if(valid){
                ...
                var html =   "<tr>"
                            ...
                            +"</td>"
                            +"<td>"
                            +"<input type='hidden' data-name='product_id' value='"+productId+"' name='receipt_others_details["+countDetail+"][product_id]'>"
                            +"<input type='hidden' data-name='qty' value='"+qty+"' name='receipt_others_details["+countDetail+"][qty]'>"
                             +"<input type='hidden' data-name='price' value='"+price+"' name='receipt_others_details["+countDetail+"][price]'>"
        ...
        $("body").on("click",".btn-delete-detail",function(e){
            ...
            $.each($(".table-input tbody tr:not(.tr-input)"),function(e,item){
                ...
                $.each($(this).find("input"),function(s,f){
                    ...
                    $(this).attr("name","receipt_others_details["+e+"]["+dataName+"]");
                })
            })
                ...
                if(validateDetail){
                    $(form).ajaxSubmit({
                        ...
                        success : function(result){
                            if(result.code == 200){
                                swal.fire('Berhasil diubah','Data penerimaan barang lainnya berhasil diubah','success');
