...
<div class="row">
    <div class="col-md-12">
        <div class="kt-portlet">
			...
			<!--begin::Form-->
            <?= $this->Form->create($receiptTransfer,['class'=>'kt-form form-primary','type'=>'file']) ?>
                                ...
                                <div class="col-md-12">
                                    <table class="table table-condensed table-bordered table-input">
                                        ...
                                        <tbody>
                                            ...
                                            <?php foreach ($receiptTransfer->receipt_transfers_details as $key => $receipt_transfers_detail):?>
                                                <tr>
                                                    <td class="number"><?=$key + 1;?></td>
                                                    <td>[<?=$receipt_transfers_detail->product->code;?>] <?=$receipt_transfers_detail->product->name;?></td>
                                                    <td class="onlyNumberWithoutComa"><?=$receipt_transfers_detail->qty;?></td>
                                                    <td class="onlyNumberPrice"><?=$receipt_transfers_detail->price;?></td>

                                                    <td class="text-center">
                                                        <input type="hidden" data-name="id" value="<?=$receipt_transfers_detail->id;?>" name="receipt_transfers_details[<?=$key;?>][id]">
                                                        <input type="hidden" data-name="product_id" value="<?=$receipt_transfers_detail->product_id;?>" name="receipt_transfers_details[<?=$key;?>][product_id]">
                                                        <input type="hidden" data-name="qty" value="<?=$receipt_transfers_detail->qty;?>" name="receipt_transfers_details[<?=$key;?>][qty]"><a href="#" class="btn btn-danger btn-delete-detail" data-id="<?=$receipt_transfers_detail->id;?>"><i class="fa fa-times"></i></a>
                                                    </td>
                                                </tr>
                                            <?php endforeach;?>
                                        </tbody>
...
<?php $this->start('script');?>
    <script>
        ...
        $(".btn-add-detail").on("click",function(e){
            ...
            if(valid){
                ...
                var html =   "<tr>"
                            ...
                            +"</td>"
                            +"<td>"
                            +"<input type='hidden' data-name='product_id' value='"+productId+"' name='receipt_transfers_details["+countDetail+"][product_id]'>"
                            +"<input type='hidden' data-name='qty' value='"+qty+"' name='receipt_transfers_details["+countDetail+"][qty]'>"
                            +"<input type='hidden' data-name='price' value='"+price+"' name='receipt_transfers_details["+countDetail+"][price]'>"
        ...
        $("body").on("click",".btn-delete-detail",function(e){
            ...
            $.each($(".table-input tbody tr:not(.tr-input)"),function(e,item){
                ...
                $.each($(this).find("input"),function(s,f){
                    ...
                    $(this).attr("name","receipt_transfers_details["+e+"]["+dataName+"]");
                })
            })
        ...
        var validatorPrimary = $(".form-primary").validate({
            ...
            messages: {
                code: {
                    required: "Harap masukan kode penerimaan barang antar lembaga"
                },
                date: {
                ...
                if(validateDetail){
                    $(form).ajaxSubmit({
                        ...
                        success : function(result){
                            if(result.code == 200){
                                swal.fire('Berhasil diubah','Datakode penerimaan barang antar lembaga berhasil diubah','success');
