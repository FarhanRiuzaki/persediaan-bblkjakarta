    ...
    <?= $this->Form->create(null, ['class' => 'm-form m-form--fit m-form--label-align-right form-primary', 'type' => 'file']) ?>
        <div class="kt-portlet__body">
            <div class="row m--margin-bottom-15">
                ...
                <div class="col-md-4">
                    <?=$this->Form->control('institute_id', ['label' => false, 'options' => $institutes, 'empty' => 'Pilih Unit Kerja', 'value' => $userData->institute_id,'style'=>'width:100%', 'class' => 'form-control']);?>
                </div>
            </div>
        </div>
...
<div class="kt-portlet m-portlet--tab">
    ...
    <?= $this->Form->create(null, ['class' => 'kt-form kt-form--fit kt-form--label-align-right form-primary', 'type' => 'file']) ?>
        <div class="kt-portlet__body">
            <div class="row kt--margin-bottom-15">
                <?php if($userData->user_group_id == 1):?>
                    <div class="col-md-3">
                        <?=$this->Form->control('category_id', ['options' => $categories, 'label' => false, 'empty' => 'Pilih Kategori', 'class' =>'form-control', 'style' => 'width:100%']);?>
                    </div>
                <?php endif;?>
                <div class="col-md-3">
                    <?=$this->Form->control('date', [
                        'autocomplete' => 'off',
                        'datepicker-report' => 'true',
                        'placeholder' => 'Bulan  ',
                        'type' => 'text',
                        'label' => false, 'class' =>'form-control', 'style' => 'width:100%'
                    ]);?>
                </div>
            </div>
        </div>
    ...
    <?= $this->Form->create(null, ['class' => 'kt-form kt-form--fit kt-form--label-align-right form-primary', 'type' => 'file']) ?>
        <div class="kt-portlet__body">
            <div class="row kt--margin-bottom-15">
                <?php if($userData->user_group_id == 1):?>
                    <div class="col-md-3">
                        <?=$this->Form->control('category_id_year', ['options' => $categories, 'label' => false, 'empty' => 'Pilih Kategori', 'class' =>'form-control', 'style' => 'width:100%']);?>
                    </div>
                <?php endif;?>
                <div class="col-md-3">
                    <?=$this->Form->control('year', ['datepicker-year' => 'true', 'placeholder' => 'Tahun', 'type' => 'text', 'label' => false, 'class' =>'form-control', 'style' => 'width:100%']);?>
                </div>
            </div>
        </div>
        <div class="kt-portlet__foot m-portlet__foot--fit">
...
<?php $this->start('script');?>
    <script>
        ...
        $(".btn-print").on("click",function(e){
            ...
            var date        = $("#start").val(),
                date2       = $("#end").val(),
                institute_id= $('#institute-id').val();
            var printData = $(this).data("print");
            if(date.length == 0 && date2.length == 0){
                swal.fire('Ooopps','Harap pilih bulan & tahun','error')
            }else if(institute_id.length == 0){
                swal.fire('Ooopps','Harap pilih Unit Kerja','error')  
            }else{
                var url = "<?=$this->Url->build(['action' => 'index']);?>?print="+printData+"&start="+date+"&end="+date2+"&institute_id="+institute_id;
                window.open(url);
                console.log(url);
            }
        ...
        $(".btn-print-month").on("click",function(e){
            ...
            if (category == '' || category == 0) {
                ...
                swal.fire('Ooopps','Harap pilih bulan','error');
            } else {
                var url = "<?=$this->Url->build(['action' => 'printMonth']);?>?print="+printData+"&category_id="+category+"&date="+date;
                window.open(url);
            }
        });
